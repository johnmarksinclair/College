class BarGraph { //Class by Dominique Meudec
  ...
  void draw() {
    ...
    if(ratio != null){
      fill(255);
      textSize(1.5*STORY_SIZE);
      text(ratio, MARGINX + 850, FIRSTY + 500); 
    }
  }
  ...
  void printTopStories(){
    int nextY = 150;
    fill(255);
    textSize(1.8*STORY_SIZE);
    text("Top 5 most popular stories by " + username, MARGINX + 5, FIRSTY + 5 + nextY - vs2.spos);
    textFont(widgetFont);
    nextY += 20;
    for (int i = 0; i < 5; i ++){
      nextY += 70;
      ...
      textSize(1.5*AUTHOR_SIZE);
      text(storyList.get(i).formattedTime, MARGINX + 45, FIRSTY + nextY + 20 - vs2.spos);
      text("Author: " + storyList.get(i).by, MARGINX + 300, FIRSTY + 20 + nextY - vs2.spos);   
    }  
  }
}
