...
class Collinear {
	 ...
	 *
	 *         Explanation: Two linear for loops with a binary search function call at the centre.
	 *
	 *
	 */
	static int countCollinearFast(int[] a1, int[] a2, int[] a3) {
		...
		for (int i = 0; i < a1.length; i++) {
			for (int j = 0; j < a2.length; j++) {
				if (binarySearch(a3, (-1*a1[i]) + (2*a2[j]))) {
					count++;
				}
			}
		}
	 ...
	 *
	 *         Explanation: Which each iteration of the while loop the search field halves, trending to the 
	 *         integer that is being searched for.
	 *
	 */
